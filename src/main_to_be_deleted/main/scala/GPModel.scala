...
object GPModel {
  ...
  val dataFemurPath = new File("data/LowerMolar/Ref/")

  def approxTotalVariance(gp: GaussianProcess[_3D, EuclideanVector[_3D]], evaluationDomain: TriangleMesh3D): Double = {
    val sampler = UniformMeshSampler3D(evaluationDomain, numberOfPoints = 1000)
  ...
  def main(args: Array[String]): Unit = {
    ...
    Seq(100, 500).foreach { i =>
      val referenceMesh = MeshIO.readMesh(new File(dataFemurPath, "mesh/ref.stl")).get

      val outputModelFile = new File(dataFemurPath, s"model/Lower_$i-components.h5")
      ...
      val cov: MatrixValuedPDKernel[_3D] = new MatrixValuedPDKernel[_3D]() {
        ...
        private val kernels = DiagonalKernel[_3D](GaussianKernel(6), 3) * 1.0
      ...
      println(s"num of components: ${lowRankGP.rank}")
