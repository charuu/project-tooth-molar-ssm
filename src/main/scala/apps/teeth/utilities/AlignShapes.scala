...
case class alignShapes(modelLms:Seq[Landmark[_3D]], sampleMeshes:Seq[(TriangleMesh[_3D],String)], sampleLms:Seq[(Seq[Landmark[_3D]],String)]) {
  ...
  private val alignedMeshesPath = new File(generalPath, "/Raw/partialMeshesAligned/meshes")
  private val alignedLMsPath = new File(generalPath, "/Raw/partialMeshesAligned/landmarks")

  def align(): Seq[((TriangleMesh[_3D],String),(Seq[Landmark[_3D]],String))] ={
    sampleMeshes.map { s =>
      val (mesh , meshName) = (s._1 ,s._2)
      val (landmarks , lmName) = sampleLms.find(l => l._2 == meshName.replaceAll("[a-z]{3}","json")).get
    ...
    //  MeshIO.writeMesh(alignedMesh, new File(alignedMeshesPath, meshName))
