...
object IcpProposalPartialDataRegistration {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //  if(mesh._1._2=="29.11.92.stl") {
        println("Targetmesh :", mesh._1._2)
        val targetMesh = mesh._1._1.operations.decimate(5000)
        ...
        val numOfSamples = 500 // Length of Markov Chain

        val proposalIcp = MixedProposalDistributions.mixedProposalPartialICPGenerator(model, targetMesh, numOfICPPointSamples, projectionDirection = ModelAndTargetSampling, 0.2, 0.2, 0.1)

        val evaluator = ProductEvaluators.proximityAndIndependent(model, targetMesh, TargetToModelEvaluation, uncertainty = 0.0001, numberOfEvaluationPoints = numOfEvaluatorPoints, "Surface")
      ...
      //  ui.show(targetGroup, alignedMeshLms.seq(0)._2._1, "landmarks")
        ...
        val bestRegistration = fitting(model, targetMesh, evaluator, proposalIcp, numOfSamples, Option(showModel), new File(logPath, s"icpProposalRegistration ${mesh._1._2.replace(".ply", "")}.json"))
