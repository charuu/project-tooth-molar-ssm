...
object ProductEvaluators {
  ...
  def proximityAndIndependent(model: StatisticalMeshModel, target: TriangleMesh3D, evaluationMode: EvaluationMode, uncertainty: Double = 1.0, numberOfEvaluationPoints: Int = 100,fitting:String="Surface"): Map[String, DistributionEvaluator[ModelFittingParameters]] = {
    //val model = m.decimate(1000)
    val likelihoodIndependent = breeze.stats.distributions.Gaussian(0, uncertainty)

    val indepPointEval = if(fitting=="Surface"){ IndependentPointDistanceEvaluator(model, target, likelihoodIndependent, evaluationMode, numberOfEvaluationPoints)}
    else{
      IndependentPointDistanceOnNormalEvaluator(model, target, likelihoodIndependent, evaluationMode, numberOfEvaluationPoints)
    }
